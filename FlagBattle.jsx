      ...
      <div style={G.root} dir="rtl">
        ...
        <section style={Object.assign({}, G.lb, { maxWidth: 760, padding: "24px 20px" })}>
          ...
          {(() => {
            ...
            const sortSearch = (list) => list.filter(c => c.name.includes(q)).sort((a, b) => {
              const as = a.name.startsWith(q), bs = b.name.startsWith(q);
              if (as !== bs) return as ? -1 : 1;
              return a.name.localeCompare(b.name, "he");
            });
            const filtConf = q ? sortSearch(confirmed) : confirmed;
            const filtProv = q ? sortSearch(provisional) : provisional;
        ...
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "36px 24px" }}>
          ...
          <div style={{ background: "rgba(196,168,79,.06)", border: "1px solid rgba(196,168,79,.2)", borderRadius: 16, padding: "32px 28px" }}>
            ...
            <div style={{ lineHeight: 1.9, fontSize: "0.95rem", color: "#c8d4e8", display: "flex", flexDirection: "column", gap: 16 }}>
              <p style={{ margin: 0 }}>בגלל שאני לא מבין כלום בסטטיסטיקה ואלגוריתמים, נתתי למערכת הקסומה שבנתה את כל זה להסביר איך היא עושה את החישוב:</p>
              <p style={{ margin: 0 }}><strong style={{ color: "#e8ecf4" }}>נקודת ההתחלה:</strong> כל עיר מתחילה עם ניקוד 0. אחרי כל קרב, הניקוד עולה למנצח ויורד למפסיד — אבל הכמות תלויה בפער ביניהם.</p>
              <p style={{ margin: 0 }}><strong style={{ color: "#e8ecf4" }}>ניצחון מפתיע שווה יותר:</strong> אם עיר חלשה מנצחת עיר חזקה — היא קופצת הרבה. אם עיר חזקה מנצחת עיר חלשה — היא כמעט לא זזה. ולהיפך בהפסד.</p>
    ...
    <div style={G.root} dir="rtl">
      ...
      <header style={G.hdr}>
        ...
        <p style={{ margin: "14px auto 0", fontSize: mob ? "1rem" : "1.2rem", fontWeight: 400, color: "#e8ecf4", textAlign: "center", lineHeight: 1.35, maxWidth: 640 }}>
          {mob ? <>
            כמעט לכל עיר, מושב וקיבוץ יש סמל.<br />
            ...
            כמעט לכל עיר, מושב וקיבוץ יש סמל. חלקם מרשימים, חלקם מזעזעים.<br />
        ...
        {(() => {
          ...
          return (
            <>
              <style>{`@keyframes lc-ticker { from { left: 0; transform: translateX(-100%) translateY(-50%); } to { left: 100%; transform: translateX(0%) translateY(-50%); } }`}</style>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, margin: "10px auto 0", maxWidth: mob ? 340 : 560 }}>
                ...
                <div style={{ overflow: "hidden", flex: 1, position: "relative", borderRadius: 20, background: "rgba(255,255,255,.06)", height: mob ? "1.6em" : "1.8em" }}>
                  {(() => {
                    ...
                    return (
                      <span style={{ position: "absolute", top: "50%", whiteSpace: "nowrap", animation: `lc-ticker ${tickerDur}s linear infinite`, fontSize: mob ? "0.8rem" : "0.95rem", color: "#e8ecf4" }}>
                        {names}
                      </span>
                    );
                  })()}
